...
- Treat a missing `ai-review:LGTM` for the current head as active work owned by the PR manager. Start an independent review through `estack:review`. Resolve actionable findings and obtain a new review verdict after every change to the PR head because each changed head invalidates the prior verdict. Continue until required verification passes and `ai-review:LGTM` certifies the current head.
