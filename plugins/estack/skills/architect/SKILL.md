...
Use a diverse panel of independent models for the Phase B runners: pick the strongest model your platform offers plus at least one different or faster one, so the perspectives stay independent. If only one provider is available, vary the model tier and give each runner a different prompt/lens so they still disagree usefully.
