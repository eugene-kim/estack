...
2. Pick the wake mechanism using your platform's loop command (for example, Claude Code's `/loop`; it's a platform feature, not a skill in this plugin). An event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
