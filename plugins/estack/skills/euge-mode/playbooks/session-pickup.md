...
1. Locate the prior trail. Use a local transcript from the active workspace or transcript location provided by the environment, a cloud-agent URL, or a pushed branch. Do not search unrelated project transcript locations; that crosses workspace boundaries and may read private chats from other projects. Read the metadata overview and the last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
