...
- When the PR platform supports labels and access permits, maintain exactly one review-status label. Set `ai-review:in-progress` when review begins. Replace it with `ai-review:changes` for unresolved actionable findings or `ai-review:LGTM` when no such findings remain. The label is the authoritative outcome; comments may explain it but never replace it. The tiers carry different authority: `ai-review:in-progress` and `ai-review:changes` are status and may be self-set. Apply `ai-review:LGTM` only as the party holding the reviewer findings behind it, never to your own implementation, which reads as self-approval to security tooling; report the outcome and leave the label to the reviewing party instead. When applying `ai-review:LGTM`, mark a complete and verified draft ready for review. If required verification remains, leave it draft; the coordinating agent remains responsible for marking it ready when verification passes.
