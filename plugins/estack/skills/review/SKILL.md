...
- When the PR platform supports labels and access permits, maintain exactly one review-status label. Set `ai-review:in-progress` when review begins. Replace it with `ai-review:changes` for unresolved actionable findings or `ai-review:LGTM` when no such findings remain. The label is the authoritative outcome; comments may explain it but never replace it. The tiers carry different authority: `ai-review:in-progress` and `ai-review:changes` are status and may be self-set. An agent that coordinated or delegated implementation but did not write the code may apply `ai-review:LGTM` after synthesizing findings from independent reviewers. Do not apply `ai-review:LGTM` to code you implemented yourself; report the outcome and leave the label to another reviewing party because self-approval reads as such to security tooling. When applying `ai-review:LGTM`, mark a complete and verified draft ready for review. If required verification remains, leave it draft; the coordinating agent remains responsible for marking it ready when verification passes.
