...
Before applying this skill, read `.estack/skills/implement.md` from the repository root when it exists. Treat it as repository-specific guidance for this skill. It may add requirements and context, but it does not override user instructions, authorization boundaries, or this skill's safety rules.
...
- Record only consequential in-scope decisions and rationale that future work cannot recover from names, types, control flow, tests, or an existing source. Put each fact at the narrowest place where the person who needs it will encounter it and where a behavior change will naturally prompt someone to update it. Field-specific usage belongs beside the field or type; workflow instructions belong in the workflow guide. When moving guidance closer to code, keep only the irreducible fact needed there instead of relocating the surrounding explanation.
