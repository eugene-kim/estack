...
Open a todolist whose first item is to read the Principles section of the **euge-mode** skill. Then add the phases below as todos.
...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with diverse, isolated, opinionated candidates and a read-only judge on a different model family (or a different tier and fresh context if your platform has one provider). Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
