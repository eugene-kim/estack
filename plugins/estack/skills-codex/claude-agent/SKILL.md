...
`/tmp/estack-claude-.../` directory for the assignment, raw stream, final result,
...
```bash
jq -r '.result' /tmp/estack-claude-.../result.json
```
...
```bash
ultrareview_run=$(mktemp -d /tmp/estack-claude-ultrareview-XXXXXX)
